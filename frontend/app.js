...
// New Therapy Module Functions
function startClaustro() {
    alert("Entering Elevator Simulation...");
}

function startDarkness() {
    alert("Entering Dark Room...");
}

// Function to handle environment navigation with auth check
function openVREnvironment(url) {
...
async function saveResultsToBackend() {
  const token = localStorage.getItem('token');
  if (!token) return;
  ...
  try {
    ...
    if (response.ok) {
      ...
      showNotification('Could not save results to server', 'warning');
    }
  } catch (err) {
